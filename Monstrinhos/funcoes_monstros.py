...
def gene_himmelblau(limite_inferior, limite_superior):
    """Sorteia um número.

    Args:
      numeros_possiveis: números possíveis de serem sorteados.

    """
    numero = random.uniform(limite_inferior, limite_superior)
    return numero


def cria_candidato_himmelblau(variaveis, limite_inferior, limite_superior):
    """Cria um candidato para o problema da Himmelblau

    Args:
      variaveis: quantidade de variaveis do problema.
      limite_inferior: limite inferior dos valores sorteados.
      limite_superior: limite superior dos valores sorteados.

    """
    candidato = []

    for _ in range(variaveis):
        candidato.append(gene_himmelblau(limite_inferior, limite_superior))

    return candidato


def populacao_himmelblau(tamanho_populacao, variaveis, limite_inferior, limite_superior):
    """Cria população inicial no problema de Himmelblau

    Args
      tamanho_populacao: tamanho da população.
      variaveis: quantidade de variaveis do problema.
      numeros_possiveis: numeros possíveis de serem sorteados.

    """
    populacao = []

    for _ in range(tamanho_populacao):
        populacao.append(cria_candidato_himmelblau(variaveis, limite_inferior, limite_superior))

    return populacao

def funcao_objetivo_himmelblau(candidato):
    """Funcao objetivo do problema de Himmelblau
...
def mutacao_simples_himmelblau(populacao, chance_de_mutacao, limite_inferior, limite_superior):
    """Realiza mutação simples adicionando perturbações aleatórias.
    ...
    for individuo in populacao:
        if random.random() < chance_de_mutacao:
            ...
            individuo[gene] += perturbacao
            individuo[gene] = max(min(individuo[gene], limite_superior), limite_inferior)

def selecao_torneio_min_himmelblau(populacao, fitness, tamanho_torneio):
    """Faz a seleção de uma população usando torneio.
    ...
    for i in range(len(populacao)):
        dados.append([populacao[i], fitness[i]])

    for _ in range(len(populacao)):
        sorteados = random.sample(dados, tamanho_torneio)

        menor_fitness = sorteados[0][1]
        individuo_selecionado = sorteados[0][0]
        
        fitness_sorteados = []
        for i in range(1, tamanho_torneio):
            if sorteados[i][1] < menor_fitness:
                menor_fitness = sorteados[i][1]
                individuo_selecionado = sorteados[i][0]
